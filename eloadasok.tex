...
\begin{document}
	...
	\begin{definition}{Osztozkodási probléma}{}
		A[0,1) egy elosztása: \\
		$[0,1) = A_1 \cup A_2 \cup ... \cup A_n$, ahol $\forall A_i$ balról zárt, jobbról nyílt páronként diszjunkt intervallumok uniója, $A_i$ véges sok
	\end{definition}
	
	A játékosok $P_1, P_2, ..., P_n \quad P_i$ értékelő eloszlás fv.-e, $f_i[0,1] \rightarrow [0,1]$, mon. növ.: $f_i(0)=0, f_i(1)=1$ \\
	
	$f_i[0,1]:=[0,x)$ intervallum értéke a $P_i$ játékos számára. $P_i$ értékelő fv.-e $\mu_i$, ahol $I = [x,y)$ esetén $\mu_i(I) = f_i(y) - f_i(x)$ és $\mu_i$ diszjunkt intervallumokon additív \\
	...
	\begin{definition}{Vágás művelet}{}
		$Vág_i(y): min{f_i^{-1}(y)}$ meghatároz $\rightarrow$ nem egyértelmű \\
		$f(x) + [f(y)-f(x)]/2$
	\end{definition}
	...
	\begin{definition}{Tasnádi rekurzív módszere}{}
		\begin{itemize}
			\item $P_i$ felosztja a [0,1)-t n egyforma részre és mindegyikhez kibocsát (n-1) jegyet
			\item Minden játékos $P_2, ..., P_n$ a számára legjobb n-1 részhez húz 1-1 jegyet
			\item Ezek után mindenkinek n-1 jegye lesz
			\item Minden részt arányosan kiosztanak a jeggyel rendelkező játékosok között T.-eljárással
		\end{itemize}
	\end{definition}
	...
	\begin{definition}{Diszkrét mozgó késes eljárás}{}
		...
		\end{itemize}
		Művelet igény $n+(n-1)+(n-2)+...+1 \Rightarrow \binom{n+1}{2}$
	\end{definition}
	...
	\begin{definition}{Elan-Paz protokoll}{}
		...
		\begin{itemize}
			\item Az $\lfloor n/2 \rfloor /n$-ik jel mentén vágunk
			\item A bal oldali részt a $\lfloor n/2 \rfloor /n$ legkisebb jelet lerakó játékos, a jobb oldalit pedig az $\lceil n/2 \rceil$ legmagasabb jelet lerakó játékos közt osztjuk szét
			\item Így a baloldalon $\lfloor n/2 \rfloor$, a jobboldalon $\lceil n/2 \rceil$ játékos osztozik arányosan az Evan-Paz eljárást követve
		\end{itemize}
		Műv. igény: O(nlogn) \\
	...
	\begin{observation}{Irigységmentes $\Rightarrow$ arányos}{}
		\begin{itemize}
			...
			\item $A_1,...,A_n$ részek adottak és $P_1,P_2,...,P_n$ sorrendben választanak 1-1 részt $\Rightarrow$ csak később választó játékos lehet irigy korábban választóra
	...
	\begin{definition}{Selfridge-Conway-eljárás}{}
		...
		\begin{enumerate}
			...
			\item $P(\overline{x})$ M-t 3 egyforma részre osztja
