...
\begin{document}
	...
	\begin{theorem}{TTC csoportosan taktikázás biztos}{}
		...
		ábra \\
		$C'_i$-ben kétféle lakás tulajdonos lehet \underline{őszinte} és \underline{hazug}. Őszinte játékos őszinte TTC szerinti cserekötésben szereplő [nem tudom elolvasni mert rózsaszínnel írtam :(] $\Rightarrow$ őszinte játékos nem jár rosszul $C'_i$-ben az őszinte TTC-hez képest. \\
	...
	\begin{definition}{Egyetemi felvételi probléma}{}
		Input: $G=(S \in J, E)$ páros gráf, ahol 
		\begin{itemize}
			\item S: szakok
			\item J: jellemzők
			\item $\forall s \in S$ szaknak van szigorúan preferált sorrendje a hozzá érkezett jelentkezésekben  $\forall j \in J$ jelentkező sorba rendezve
			\item $\forall s \in S$ szakhoz adott $q_s$
		\end{itemize}
	\end{definition}
	
	\begin{definition}{Felvételi séma}{}
		$F \leq E$, amire $\forall i \in J$-ből $\leq 1$ F-beli él indul. $\forall s \in S$ szakból $\leq q_s$ indul \\
		$e = j_s$ él blokkolja felvéltelek számát, ha
		\begin{itemize}
	...
	\begin{statement}{M párosítás G'-ben}{}
		M vetülete G-ben felvételi séma, ahol M vetülete $P(M)={j_s : \exists j_s \in M}$ \\
		$P(M) \forall j \in J \leq 1$ élt, $\forall s \in S$-ből [valami]
	\end{statement}
	...
	\begin{Ponthatárcsökkentő algoritmus}{}
		...
	\end{definition}
