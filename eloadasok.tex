...
\begin{document}
	...
	\section{Előadás [Kombinatorikus játék]}
	...
	\newpage 
	\section{Előadás [Sprague-Grundy, izomorfia, ekvivalencia]}
	...
	\begin{theorem}{Bouton}{}
		k-nim II-es típusú $\Leftrightarrow n_1 \oplus n_2 \oplus ... \oplus n_k = 0$, ahol $n_1, ... n_k$ a kupacok méretei.
	\end{theorem}
	...
	\end{statement}
	Biz.: \\
	Tdh. indirekten, hogy a 2. játékosnak van nyerő stratégiája. Ekkor az 1. játékos az első lépését kezelje úgy mintha nem lenne a táblán, és játsszon a II. játékos nyerő stratégiájával a saját irányába változtatva. Ha a strat. szerint rejtett mezőre lépne, tegyen egy másik tetszőleges mezőre, és onnatól kezelje azt rejtettként. A játék végén mindkét játékos nyertes stratégiát követett, tehát mindketten úgy látják nyertes. Ez nem lehetséges $\Rightarrow$ ellentmondás $\Rightarrow$ I-es típusú a hex.
	...
	\newpage
	\section{Előadás [Tiszta Nash-egyensúly]}
	...
	\begin{definition}{Erős dominálás}{}
		Az $i.$ játékos $z \in S_i$ stratégiája erősen dominálja a $z' \in S_i$ stratégiáját, ha a többi játékos tetszőleges $s_1 \in S_1, ..., s_{i-1} \in S_{i-1}, s_{i+1} \in S_{i+1}, ..., s_n \in S_n$ stratégiaválasztás esetén  
		\begin{center}
	...
	\begin{cases}
		...
		u_1(p, \sigma_2) = 1/3(1+0-1)=0 \\
		u_1(o, \sigma_2) = 1/3(-1+1+0)=0
	\end{cases}
	\]
	...
	\begin{figure}[h]
		...
		\begin{minipage}{0.45\textwidth}
			...
			\begin{tikzpicture}[scale=1]
				...
				\draw[dotted,step=1cm,gray,very thin] (0,0) grid (2,4);
				...
				\draw [very thick](0,3) -- (1,0);
				\draw [green,very thick](0.5,0) -- (0.5,1.5);
		...
		\begin{minipage}{0.45\textwidth}
			...
			\begin{tikzpicture}[scale=3]
				...
				% rácsvonalak (opcionális)
				\draw[dotted,step=1cm,gray,very thin] (0,0) grid (1,1);
				...
				%line
				\draw[green,very thick] (0,1) -- (0.5,1);
				\draw[green,very thick] (0.5,1) -- (0.5,0);
	...
	\begin{figure}[h]
		...
		\begin{minipage}{0.45\textwidth}
			...
			\begin{tikzpicture}[scale=1]
				...
				% rácsvonalak (opcionális)
				\draw[dotted,step=1cm,gray,very thin] (0,0) grid (2,5);
				...
				\draw [green,very thick](0.333,0) -- (0.333,1.333);
		...
		\begin{minipage}{0.45\textwidth}
			...
			\begin{tikzpicture}[scale=3]
				...
				%line
				\draw[green,very thick] (0,0) -- (0.333,0);
				\draw[green,very thick] (0.333,0) -- (0.333,1);
				\draw[green,very thick] (0.333,1) -- (1,1);
	...
	\begin{figure}[h]
		...
		\begin{minipage}{0.45\textwidth}
			...
			\begin{tikzpicture}[scale=3]
				...
				\draw[green,very thick] (0,1) -- (0,0.5);
				\draw[green,,very thick] (0,0.5) -- (1,0.5);
				\draw[green,very thick] (1,0.5) -- (1,0);
	...
	\newpage
	\newpage
	\section{Előadás [Lineáris programozási feladat, csődjáték, kelmeszabály]}
	...
	\begin{definition}{Kaminski-féle közlekedőedény-rendszer}{}
		...
		\href{https://cs.bme.hu/~nemkin/jatek/hydra/}{Hidraulikus közlekedőedények}
		\begin{center}
	...
	\begin{center}
		\includegraphics{res/nukleusz.png}
	\end{center}
	...
	\newpage
	\section{Előadás [Osztozkodási játék]}
	...
	\begin{definition}{Igazságos felosztás}{}
		Akkor definiáljuk igazságosnak a mechanizmust, ha mindegyik
		játékos számára garantálható, hogy a végén neki jutó $A_i$ darabra $\mu_i(A_i) \geq \mu_i(P)/n$, vagyis a saját
		mértéke szerint legalább az n-ed részt kapja, még akkor is, ha az összes többi játékos összefogna
		ellene. 
	\end{definition}
	...
	\begin{definition}{Fink eljárás}{}
		...
		\begin{itemize}
			...
			\item Tehát az eljárás igazmondásra ösztönöz, igazságos, arányos és véges
		\end{itemize}
	\end{definition}
	\begin{comment}
	...
	\begin{definition}{Tasnádi rekurzív módszere}{}
		\begin{itemize}
			...
			\item igazmondásra ösztönöz, igazságos
		\end{itemize}
	\end{definition}
	...
	\begin{definition}{Diszkrét mozgó késes eljárás}{}
		...
		A mozgó késes eljárás igazságos
	\end{definition}
	...
	\begin{definition}{Irigységmentesség}{}
		...
		mindig van irigységmentes felosztás, ami ráadásul összefüggő
		szeletekből áll
	\end{definition}
	...
	\newpage
	\section{Előadás [Szavazási modell(TVSZ, TVT)]}
	...
	\begin{definition}{Szavazási modell}{}
		Legyen $N={1, ..., n}$ a szavazók halmaza és $A = \{a_1, ..., 1_k\}$ az alternatívák halmaza. Az $i.$ szavazó szavazata az alternatívák egy lehetséges sorrendje (permutációja), az ehhez tartozó rendezési reláció jele a $\prec_i$. Az $n$ szavazó egy lehetséges választási profilja a $\Pi = (\prec_1, ..., \prec_n)$. A szavazási mechanizmus egy $F(\Pi)$ társadalmi választási szabályt (TVSZ) határoz meg, mely egy választási profilhoz hozzárendeli a szavazás eredményét, azaz az alternatíváknak a közös döntés szerinti sorrendjét. A nyertes alternatíva a TVSZ sorrendjében az első, az $f(\Pi)$ társadalmi választási függvény (TVF) ezt a nyertes alternatívát rendeli hozzá a $\Pi$-hez.
	\end{definition}
	
	\begin{definition}{Jóváhagyásos szavazás}{}
		A jóváhagyásos szavazás az az eljárás, amelyikben minden szavazó tetszőleges	számú alternatívát jelölhet meg, és a nyertes alternatíva az, amelyiket a legtöbben megjelölték.
		(Döntetlen esetén az a legtöbb szavazatot kapó alternatívák közül a $\preceq_1$ szerinti legjobb.) \\
		A jóváhagyásos szavazás nem TVF, mert nem kizárólag a szavazók preferenciarendezéseitől
		függ a kimenetel. A jóváhagyásos szavazás tehát nem fér bele a fenti modellbe.
	\end{definition}
	...
	\begin{definition}{Borda-szavazás}{}
		Az $i.$ szavazó $a \in A$ alternatívához rendelt Borda-pontszáma $B_i(a) = k - l$, ahol $l$ az $a$ alternatíva sorszáma az $i.$ szavazó sorrendjében, az alternatívák száma pedig $k$. Az $a$ Borda-pontszáma $B(a)$, a szavazók Bordapontszámainak az összege. A Borda-szavazás TVSZ-e a Borda-pontszám szerinti csökkenő sorrendbe rendezi az alternatívákat. Egyenlőség esetén az első szavazó szavazata dönt.
	\end{definition}
	
	\begin{definition}{Condorcet-konzisztencia}{}
		Az $a$ alternatíva legyőz egy tőle különböző $b$ alternatívát, ha szigorúan több szavazó sorolta $a$-t a $b$ elé a sorrendjében. Az $a$ alternatíva \textbf{Condorcet-győztes} , ha legyőzött minden más alternatívát (ilyen nem mindig van). Egy szavazási mechanizmus Condorcet-konzisztens, ha a Condorcet-győztest választja nyertesnek, amennyiben az létezik.
	\end{definition}
	...
	\begin{definition}{Elérhető}{}
		Az f TVF és $\pi$ választási profil mellett az $a \in A$ alternatíva az $i$ szavazó számára $\pi$ elérhető, ha van olyan $\preceq \in L$ rendezés az alternatívákon, amire $f(\pi_-i, \preceq)=a$. Az i számára $\pi$ elérhető alternatívák halmazát $A(\pi,i)$ jelöli.
	\end{definition}
	
		\begin{definition}{Háromféle szavazatmanipuláció}{}
		\begin{itemize}
			\item Átrendezés ($\preceq_i^A$): A szavazó úgy rendezi át a listáját, hogy a közös döntést ($f(\Pi)$) pontosan azok az alternatívák előzzék meg, amik az eredeti listáján is előrébb voltak, de ezek belső sorrendje tetszőlegesen változhat.
			\item Feljavítás ($\preceq_i^F$): A szavazó a közös döntést ($f(\Pi)$) "előrébb sorolja" a listáján, miközben az összes többi alternatíva egymáshoz képesti sorrendje változatlan marad.
			\item Jobbra shiftelés ($\preceq_i^J$): Egy olyan alternatívát ($a$), amely a szavazó számára rosszabb, mint a közös döntés, és amit egyedül nem is tudna győztessé tenni ($a \notin A(\Pi, i)$), szándékosan a közös döntés elé helyez.
		\end{itemize}
	\end{definition}
	
	Következmény. Ha az $f$ TVF taktikázásbiztos és a $\Pi$ választási profilt átrendezések,feljavítások és jobbra shiftelések tetszőleges sorozata $\Pi'$-re módosítja, akkor $f(\Pi) = f(\Pi')$.
	
	\begin{definition}{Jobbra shiftelt profil}{}
		A $\Pi$ választási profilt jobbra shiftelt profilnak nevezzük, ha egyetlen szavazó sem tud a $\Pi$-beli szavazatán jobbra shiftelni.	A $\Pi$ választási profilt akkor mondjuk teljesen jobbra shiftelt profilnak, ha minden, a$\Pi$-ből átrendezések sorozatával kapható $\Pi'$ profil jobbra shiftelt
	\end{definition}
	Egy jobbra shiftelt $\Pi$ profilban tehát minden szavazat az adott szavazó számára elérhetetlen
	alternatívákkal kezdődik, ezt követi az $f(\Pi)$ közös döntés, majd az adott szavazó számára $\Pi$ elérhető alternatívák zárják a sort. Megtörténhet, hogy egy jobbra shiftelt profilban az i szavazó
	szavazatának átrendezése nyomán egy j szavazó számára egy korábban elérhető a alternatíva
	elérhetetlenné válik. Mivel ez az a alternatíva a j szavazó szavazatában a közös döntést követi,
	ezért j képes a szavazatát az említett átrendezés után jobbra shiftelni. Átrendezések és jobbra
	shiftelések egymásutánjával azonban tetszőleges $\Pi$ profil teljesen jobbra shifteltté tehető
	...
	\newpage
	\section{Előadás [Vickrey-árverés]}
	...
	\begin{definition}{M mechanizmus taktikázás biztos}{}
		Ha $u_i(\underline{v}) \leq u_i(\underline{v}_{-i},\hat{v}_i) \quad \forall \underline{v} \in S \quad \forall i \in N$
	\end{definition}
	
	\begin{definition}{Vickrey-Clarke-Groves (VCG) mechanizmus osztály}{}
		\begin{itemize}
			...
			$\rightarrow$ A mechanizmus olyan kimenetelt választ, amelynél a résztvevők által bemondott értékek összege a lehető legnagyobb.
			...
			$\rightarrow$ Minden résztvevő annyit fizet, amennyi "kárt" okoz a többi résztvevőnek a jelenlétével.
		\end{itemize}
	\end{definition}
	...
	\begin{definition}{M szubvenciómentes}{}
		Ha $p_i \geq 0 \quad \forall i \rightarrow$  ha egy licitáló őszintén licitál, akkor nem érheti veszteség.
	\end{definition}
	...
	\begin{definition}{Clarke-szabály}{}
		 i alapkifizetése $h_i(\underline{v}_{-i}) = max\{ \sum_{i \neq j} v_j(a): a \in A\}$
	\end{definition}
	...
	\newpage
	\section{Előadás [Újraosztási feladat]}
	...
	\begin{definition}{Újraosztási feladat}{}
		$N = \{1, 2, ..., n\} \quad \preceq_i \in P = \{1, 2, ..., n\}$-en szigorú preferencia rendezések $\forall i \in N$ \\
	...
	\begin{definition}{Gyengén blokkolás}{}
		$\sigma \in S_N$-et a $B \subseteq N \quad$ gyengén blokkolja, ha $\exists \pi \in S_B$, amire a $\sigma(i) \preceq_i \pi(i), \forall i \in B$ és $\exists j \in B :  \sigma(j) \preceq_j \pi(j) $\\
		$\sigma$ erős mag, ha $\nexists$ gyengén blokkoló koalíció. \\
		Lakástulajdonosok egy $B \subseteq N$halmaza a $\sigma$ újraelosztást gyengén blokkoló koalíciót alkot,
		ha a B-beli tulajdonosok el tudják osztani a saját lakásaikat egymás között úgy, hogy senki se
		járjon rosszabbul, mint a $\sigma$ újraelosztás mellett, és legalább egy B-beli tulajdonos helyzete határozottan javuljon $\sigma$-hoz képest. 
	\end{definition}
	
	\begin{definition}{Erős mag}{}
		$\sigma$ erős mag, ha $\nexists$ gyengén blokkoló koalíciója
	\end{definition}
	
	\begin{definition}{}{}
		$S \subseteq N \quad D(S)$ irányított segédgráf csúcsai: $S \forall i $ csúcsból 1-1 irányított él fut abba a csúcsba, amelyik lakását i a legjobban preferálja az S-beliek közül.
	\end{definition}
	...
	\begin{theorem}{Lemma}{}
		Ha $S \neq \emptyset$ $\Rightarrow D(S)$ tartalmaz irányított kört (diszjunkt körök halmaza)
	\end{theorem}
	...
	\begin{definition}{Manipulálhatóság}{}
	...
	\begin{definition}{Taktikázásbiztosság}{}
		f csoportosan taktikázásbiztos, ha nem csoportosan manipulálható.
	\end{definition}
	
	\begin{theorem}{TTC taktikázásbiztossága}{}
		TTC csoportosan taktikázásbiztos
	\end{theorem}
	...
	\begin{definition}{Piaci egyensúly}{}
		...
		\begin{itemize}
			\item $p (\sigma(i)) \leq p(i) \quad \forall i$
			\item $\sigma(i) \preceq_i j \Rightarrow p(i) < p(j)$
		\end{itemize}
		Minden lakáshoz hozzárendelek egy (pozitív) valós számot (árat) úgy, hogy
		ha minden tulajdonosnak adok annyi pénzt, amennyit a saját lakása ér, majd mindenkit megkérek, hogy mutasson rá a neki legszimpatikusabb lakásra aminek az árát ki tudná fizetni, akkor minden lakásra pontosan egyvalaki mutatott rá (vagyis ez meghatároz egy újraosztást).
	\end{definition}
	...
	\newpage
	\section{Előadás [Stabil párosítások]}
	Stabil párosítások, blokkoló koalíció, blokkoló él. Éltörlési lemma, lánykérő algoritmus, fiú/lány-optimális stabil párosítás. Stabil párosítás által fedett csúcshalmaz invarianciája.
	
	\begin{definition}{Stabil párosítás}{}
		Adott a $G = (V, E)$ gráf és minden $v \in V$ csúcsához a $v$-re illeszkedő élek $E(v)$
		halmazán a $v$ csúcs preferenciáját leíró $\preceq_v$ lineáris rendezés. Azt mondjuk, hogy $v$ számára az
		$e$ él jobb az $f$ élnél, ha $f \preceq_v e$. A G gráf M párosítása mellett az $uv$ élt akkor nevezzük tehát
		blokkoló élnek, ha $u$ és $v$ is jobban jár az $uv$ éllel, mint $M$-mel, azaz ha $M$ nem tartalmaz
		olyan élt sem, ami $u$ számára és olyan élt sem, ami $v$ számára jobb az $uv$ élnél. Ha az $M$
		párosítás mellett nincs blokkoló él, akkor $M$-et stabil párosításnak nevezzük
	\end{definition}
	
	\begin{definition}{Blokkoló koalíció}{}
		content...
	\end{definition}
	
	\begin{definition}{Blokkoló él}{}
		content...
	\end{definition}
	
	\begin{definition}{Éltörlési lemma}{}
		content...
	\end{definition}
	
	\begin{definition}{Gale és Shapley alábbi lánykérő algoritmus}{}
		Legyen $G = (V, E)$ irányítatlan gráf és legyen $w : E \rightarrow \mathbb{R}$ egy olyan
		élsúlyozás, ahol nincs két egyforma súlyú él. A csúcsok $\preceq_v$ élpreferenciáit úgy definiáljuk, hogy
		tetszőleges v-re illeszkedő e, f élek esetén $f \preceq_v e$ ha $w(e) < w(f)$.
		\begin{enumerate}
			\item Minden fiú megkéri a számára legszimpatikusabb lány kezét.
			\item Minden lány a legjobb kivételével kikosarazza az összes kérőjét.
			\item Ha nem volt kikosarazás, akkor STOP: a lánykérésekhez tartozó párosítás az output.
			\item Ha volt kikosarazás akkor GoTo 1
		\end{enumerate}
		Fiú-optimális és a fiúk számára taktikázásbiztos.
	\end{definition}
	
	\begin{theorem}{Gale–Shapley-tétel}{}
	Ha $G$ páros gráf, és $\preceq_v$ lineáris rendezés $E(G)$-n minden
	$v \in V$ esetén, akkor G-nek van stabil párosítása.
	\end{theorem}
	
	\begin{theorem}
		Ha $M_1$ és $M_2$ az élpreferenciákkal ellátott (nem feltétlenül páros) $G$ gráf két stabil
		párosítása, akkor $V (M_1) = V (M_2)$, azaz a $G$ gráf bármelyik stabil párosítását is választjuk,
		mindig ugyanazok a csúcsok lesznek kipárosítva, és ugyanazok a csúcsok maradnak fedetlenül.
	\end{theorem}
	
	\begin{definition}{Stabil párosítás által fedett csúcshalmaz invarianciája}{}
		content...
	\end{definition}
	
	\newpage
	\section{Előadás [Egyetemi felvételi probléma, vonalhúzási probléma]}
	...
	\newpage
	\section{Előadás [Népszerű párosítások]}
	...
	\begin{definition}{Népszerű párosítások}{}
		G=(V,E) gráf, M, N a G gráf párosításai, N $\prec$M, ha M-nek több szavazója van, mint N-nek: $|\{v \in V: N \prec_v M \}|>|\{v\in V:M\prec_v N\}|$
	\end{definition}
	
	\begin{observation}{}{}
		M népszerű, ha $\nexists$ N párosítás, amire M$\npreceq$N
	\end{observation}
	
	\begin{theorem}{Gördenfors}{}
		Minden stabil párosítás népszerű
	\end{theorem}
	Biz.:\\
	Tfh. M stabil és N tetszőleges párosítás G-ben. Cél:M$\nprec$N (M-nek legalább annyi szavazója van, mint N-nek)\\
	N szavazói: N$\backslash$M-beli élek bizonyos végpontjai\\
	Ha $uv \in$(N$\backslash$M) és N-re szavaz, akkor v M-re szavaz, különben uv blokkolná M-et $\Rightarrow$ ellentmondás!
	
	\begin{statement}{}{}
		Ha M stabil $\Rightarrow$ M min méretű népszerű párosítás
	\end{statement}
	
	\begin{theorem}{Kavitha-algoritmus}{}
		Lánykérő-algoritmust futtatunk, de ha egy fiút minden lány elutasított, akkor "személyiségfejlesztő" trainingre küldjük. Ekkor minden lány minden olyan fiúnál jobbnak fogja gondolni, mint aki nem volt trainingen.\\
		Kavitha algoritmusa $G^K$ fiú-opt. stabil párosítást találja meg.
	\end{theorem}
	
	\begin{theorem}{}{}
		Ha M a $G^K$ stabil párosítása $\Rightarrow$ M-nek megfelelő élek G-ben max méretű párosítást alkotnak
	\end{theorem}
	...
	\begin{itemize}
		...
		\item \href{https://www.cs.bme.hu/~fleiner/alje2025/segedanyag.pdf}{Dr. Fleiner Tamás Online segédanyag}
