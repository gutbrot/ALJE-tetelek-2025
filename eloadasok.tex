...
\begin{document}
	...
	\newpage
	\section{Előadás}
	Lásd "Szavazási eljárások diasor"
	
	\newpage
	\section{Előadás}
	\begin{definition}{Árverási model}{}
		$N = \{1, 2, ..., n\}$ licitálók $A = \{a_1, ..., a_k\}$ \\
		...
		$S = S_1 \times ... \times S_n$ S elemi a licitprofilok
	\end{definition}
	...
	\begin{definition}{Árverési mechanizmus}{}
		$f: S \rightarrow A$ a győztes alternatívát kiválasztó a fv. és $p_i: S \rightarrow \mathbb{R}$ mutatja meg, hogy az i-dik licitáló mennyit fizet az $f(\underline{v})$ alternatíva győzelme esetén
	\end{definition}
	...
	\begin{definition}{Vickrey-Clarke-Groves (VCG)}{}
		\begin{itemize}
			\item $f(\underline{v}) maximalizálja \sum_{i=1}^{n} v_i (a) \in A$-ra $\sum_{i=1}^{n} v_i (f(\underline{v})) \geq \sum_{i=1}^{n} v_i(a) \quad \forall a \forall \underline{v} \in S$
			\item $p_i(\underline{v}) = h_i(\underline{v}-i) - \sum_{j \neq i} v_j (f(\underline{v})) $ teljesül, ahol tetszőleges $h_i: S_{-i} \rightarrow \mathbb{R}$
			%% $\forall \underline(v) $\forall$ i$
			%% ezt se tudom hova jön
		\end{itemize}
	\end{definition}
	
	\begin{theorem}{VCG taktikázás biztos $\forall h$-ra}{}
		Biz.:
	\end{theorem}
	...
	\begin{definition}{M veszteségmentes}{}
		Ha $ \forall i \in N \quad \forall \underline{v} \in S u_i(\underline{v}) \geq 0$, ha $v_i = \hat{v}_i$
	\end{definition}
	...
	\begin{definition}{Újraosztási feladat}{}
		$N = {1, 2, ..., n} \preceq_i \in P = {1, 2, ..., n}$-en szigorú pref. rendezéssel $\forall i \in N$ \\
		$\Pi = (\preceq_1, ..., \preceq_n) \in P^n$ pref. profil \\
	...
	\begin{definition}{}{}
		$\sigma \in S_N$-et a $B \subseteq N \quad \underline{gyengén blokkolja}$, ha $\exists \pi \in S_B$, amire a $\sigma(i) \preceq_i \in B$ és $\exists j \in B \quad \sigma(j) \preceq_j \pi(j) | \sigma \in $ erős mag, ha $\nexists$ gyengén blokkolók. 
	\end{definition}
	
	\begin{definition}{}{}
		$S \subseteq N D(S)$ segédgráf csúcsai: $S \forall i $ csúcsból 1-1 irányított él fut abba a csúcsba, amelyik lakását i a legjobban preferálja az S-beliek közül
	\end{definition}
	...
	\begin{definition}{Top Trading Cycles algoritmus}{}
		\begin{enumerate}
			...
			\item $N_i$-beli játékosokra definiáljuk a $\sigma$-t a $D(S-i)$-ben
			\item $i = i+1$ GOTO 2. lépés
		\end{enumerate}
	\end{definition}
	
	\begin{remark}{}{}
		$\sigma$ a TTC outputja!
	\end{remark}
	
	Példa a működésre: \url{https://blockchain-academy.hs-mittweida.de/courses/game-theory-blockchain/lessons/cooperative-game-theory/topic/top-trading-cycle-algorithm/}
	
	\begin{theorem}{Lemma}{}
		ha $s \neq$ $\Rightarrow D(S)$ tartalmaz irányított kört [diszjunkt körök] = $D(S)$-beli irányított körök csúcshalmaza 
	\end{theorem}
	
	\begin{definition}{}{}
		j játékos rangja i, ha $j \neq N_i$
	\end{definition}
	...
	\begin{theorem}{Lemma}{}
		...
		Ha j a saját rangjánál nagyobb rangú játékos lakását kapja $\Rightarrow \sigma$-hoz képest rosszabbul jár 
	\end{theorem}
	...
	\begin{definition}{f újraosztási mechanizmus csoportosan manipulálható}{}
		content...
	\end{definition}
	
	\begin{definition}{f csoportosan taktikázásbiztos}{}
		content...
	\end{definition}
	Biz*.: [előadáson nem fejeztük be]
	
	\begin{definition}{Piaci egyensúly}{}
		$p: N \rightarrow \mathbb{R}$ ár függvény és $\sigma \in S_N$ a $(p, \sigma) \quad P-l, ha$
		\begin{itemize}
	...
	\begin{statement}{}{}
		$(p, \sigma) PE \Rightarrow \sigma$ erős magbeli 
	\end{statement}
	Biz.:
	
	\begin{observation}{}{}
		$\sigma$-beli cserekörök mentén a lakások árait nem változtatjuk \\
		$\pi(i) <_j \sigma(i) \Rightarrow$ nem blokkol gyengén $\rightarrow$ ellentmondás!
	\end{observation}
	
	\begin{statement}{}{}
		$\sigma$ a TTC outputja $\Rightarrow \exists p : N \rightarrow \mathbb{R}$, amire $(p, q)$ [???]
	\end{statement}
	...
	\begin{itemize}
		...
		\item Nemkin Viktória feladat megoldások \url{https://cs.bme.hu/~nemkin/jatek/}
